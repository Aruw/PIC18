...
// Caracteres enviados pela porta serial.
#define R	0x8A
#define P	0x8B
#define S	0x8C
#define	E	0x8D
...
#define	LED	LATBbits.LATB3					//	LED que informa proximidade do setpoint.

#define	BT7	PORTBbits.RB2					//	Botão para exibir o argumento do PWM no display de LEDs.


#define	TAXA_ATUALIZA_PWM	50				//	Delay para atualização do PWM.
...
void main( void )
 {
	...
	// Configuração da porta serial RS-232:
	OpenUSART( 	USART_TX_INT_OFF 	&	// Interrupção na transmissão desabilitada.
			   	USART_RX_INT_OFF 	&	// Interrupção na recepção desabilitada.
			   	USART_ASYNCH_MODE 	&	// USART em modo assíncrono.
				USART_EIGHT_BIT 	&	// Dados de 8 bits.
				USART_CONT_RX 		&	// Recepção contínua.
				USART_BRGH_HIGH,		// Alta velocidade.
				 25 );					// Baud-Rate de 9600 bps ( aproximadamente ).
	...
	lcd_rom_puts( "REF MOTOR    PWM" );	//	Escreve na linha superior.
	...
	while ( 1 )								// Loop infinito.
	 {
		if ( SHOW_LCD )						//	Se houver permissão para atualizar o display de LCD.
		 {					
			...
			converte_bcd( ref_vent );		//	Obtém os 3 dígitos decimais da referência ( setpoint ).
			...
			Duty_Cycle=	( unsigned char ) ( ( 100.0 * ( float ) intensidade_vent ) / ( float ) Periodo_PWM ); 
			...
			if ( atualizar_PWM-- == 0 )			
			{
				...
				erro=	cont_vent - referencia;				//	Calcula o erro na regulagem da velocidade.
			
				if ( erro > 300 ) erro= 300;				//	Satura o erro para evitar instabilidade.
				if ( erro < -300 )erro=-300;

				intensidade_vent-= erro / 5; 				//	Desconta um valor proporcional ao erro.

				if ( abs( erro ) < 5 )
				{
					if ( erro < 5 && erro > 0 )
					{
						if ( rand() % 9 == 0 ) intensidade_vent--;
					}
				...
				// Saturação para variações abruptas da variável manipulada. Tende a reduzir instabilidades.			
				if ( intensidade_vent - intensidade_vent_anterior > 15 )
					intensidade_vent= intensidade_vent_anterior + 15;

				if ( intensidade_vent - intensidade_vent_anterior < -15 )
					intensidade_vent= intensidade_vent_anterior - 15;


				intensidade_vent_anterior=	intensidade_vent;
		...
		LED=	( erro < 5 ) && ( erro > -5 );				// LED aceso quando a variável controlada
															// estiver muito próxima do setpoint.
		
		ref_vent=	ref_bt / 100;							// Referência ( setpoint ), definido pelo contador dos botões.
		referencia=	ref_vent * 7;							// Ajustando a referência computacional pelo número de pás.

		//	if ( ref_vent < 10 )	ref_bt= 1000;			// Limita o setpoint mínimo, para não haver parada.
		...
		while ( BusyUSART() );								// Aguarda a transmissão corrente da USART encerrar.

											// Seleciona um dos bytes abaixo.

		switch ( charcounter++ )
 		 {
			case 0:	usart_tx=	R;	break;
			case 1:	usart_tx=	P;	break;
			case 2:	usart_tx=	S;	break;
			case 3:	usart_tx=	( unsigned char ) cont_vent_round; break;
			case 4:	usart_tx=	R;	break;
			case 5:	usart_tx=	E;	break;
			case 6:	usart_tx=	F;	break;
			case 7:	usart_tx=	( unsigned char ) ref_vent; break;
			case 8:	usart_tx=	P;	break;
			case 9:	usart_tx=	W;	break;
			case 10:usart_tx=	M;	break;
			case 11:usart_tx=	( unsigned char ) Duty_Cycle;	break;
			default:	charcounter= 0;			 
		 }

		WriteUSART( usart_tx );			// Transmite o byte pela porta-serial.
